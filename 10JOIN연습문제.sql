...
--HINT) 어떤 컬럼으로 서로 연결되 있는지 확인
  ...
  SELECT CONCAT(E.FIRST_NAME || ' ', E.LAST_NAME) AS 이름, E.JOB_ID, J.JOB_TITLE
  FROM EMPLOYEES E INNER JOIN JOBS J ON E.JOB_ID = J.JOB_ID ORDER BY 이름;
  
--문제 4.
--JOBS테이블과 JOB_HISTORY테이블을 LEFT_OUTER JOIN 하세요.
  
  SELECT * FROM JOBS JS LEFT OUTER JOIN JOB_HISTORY JH ON JS.JOB_ID = JH.JOB_ID;
