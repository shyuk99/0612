...
CREATE TABLE DEPTS(
    ...
    DEPT_NAME VARCHAR(30), -- 30BYTE (한글은 15글자, 숫자 영어는 30글자)
    DEPT_YN CHAR(1), -- 고정문자 1BYTE(VARCHAR2 대체가능)
    DEPT_DATE DATE, -- 날짜
    DEPT_BONUS NUMBER(10, 2), -- 정수 10자리, 소수점 2자리수 까지 저장
    DEPT_CONTENT LONG -- 2GB까지 저장되는 가변 문자열 (VARCHAR2보다 더 큰 - DB마다 이름이 조금씩 다름)
);
...
INSERT INTO DEPTS VALUES(1, 'HELLO', '가', SYSDATE, 3.14, 'LONG~TEXT~');
...
ALTER TABLE DEPTS RENAME COLUMN DEPT_COUNT TO EMP_COUNT; -- 컬렴명 변경
...
DROP TABLE DEPARTMENTS; -- DEPARTMENT는 EMPLOYEES테이블과 참조관계를 가지고 있어서, 한번에 삭제되지 않음.
