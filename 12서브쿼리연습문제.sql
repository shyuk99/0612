...
-- EMPLOYEES테이블에서 department_id(부서아이디) 가 일치하는 모든 사원의 정보를 검색하세요.
   ...
   SELECT *
   FROM (SELECT * FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
...
-- Steven의 급여보다 많은 급여를 받는 사람들은 출력하세요.
   ...
   SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID IN (SELECT * FROM EMPLOYEES WHERE FIRST_NAME = 'Steven');
...
-- 문제 4의 결과를 (스칼라 쿼리)로 동일하게 조회하세요
   ...
   SELECT *
   FROM (SELECT ROWNUM AS RN, E. * FROM (SELECT * FROM EMPLOYEES ORDER BY FIRST_NAME DESC) E)
