-- 뷰
-- 뷰는 제한적인 데이터를 쉽게 보기 위해서 미리 만들어놓은 가상테이블
-- 자주 사용되는 컬럼을 저장하면, 관리가 용이함
-- 뷰는 물리적으로 데이터가 저장된 것은 아니고, 원본테이블을 기반으로 한 가상테이블이라고 생각하면 됨
-- 뷰를 만드려면 권한이 필요함 (HR)은 권한을 가지고 있음
...
-- 단순뷰는 뷰를 통해서, INSERT, UPDATE가 가능한데, 제약사항들이 있음
-- 복합뷰는 INSERT, UPDATE가 불가능함
SELECT * FROM VIEW_EMP;
